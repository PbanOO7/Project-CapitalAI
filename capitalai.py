...
def calculate_portfolio_metrics() -> dict:
    ...
    closed = trades[trades["status"] == "CLOSED"].copy() if not trades.empty else trades.iloc[0:0]
    active = trades[trades["status"] == "ACTIVE"].copy() if not trades.empty else trades.iloc[0:0]
